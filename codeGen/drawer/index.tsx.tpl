...
import { ACTIVE_STATUS } from '@/consts'
...
import %=typeName=%Detail from './detail';
import %=typeName=%Print from './print';
...
const %=name=%Index:React.FC<Props> = (props) =>{
  ...
  // 更改每页展示条数
  const handleChangePageSize = async(page: number, pageSize: number) => {
    await dispatch({
  ...
  // 展示弹框
  const handleModal = (record: %=typeName=%, actionType: any) => {
    dispatch({
      type: '%=nameSpace=%/updateState',
      payload: {
        currentItem: record,
