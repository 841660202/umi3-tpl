import { %=typeName=%State } from './type';
import { fetchSave, fetchDelete, fetchList } from './service';

const %=nameSpace=%: %=typeName=%State = {
  namespace: '%=nameSpace=%',
  state: {
    data: [], // 列表
    loading: false,
  ...
  effects: {
    ...
    *loadList({ payload }, { call, put, select }) {
      const state = yield select((state: %=typeName=%State) => state.%=nameSpace=%);
      const { pageIndex, pageSize, query } = state;
      const params = { pageIndex, pageSize, ...query, ...payload };
      yield put({
        type: 'updateState',
